...
消费位移的**自动提交是在 poll() 方法里完成的**，在每次真正向服务端发起拉取请求之前会检查是否可以进行位移提交，它非常简便，
但是与此同时的是重复消费和消息丢失的问题。
...
查看`MyRebalanceListener`类查看具体实现
...
也就是说它会每隔5秒拉取到每个分区中最大的消费位移，但是它的提交动作是在`poll()`方法逻辑里，在每次向服务端拉取消息之前会检查是否可以进行位移提交。
